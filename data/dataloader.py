...
def load_dataloader(max_length=128,batch_size=64,mask_token=50264,mask_prob=0.3):
    ...
    train_dataset = tokenize_dataset(train_dataset,tokenizer,max_length,mask_token,mask_prob)
    val_dataset   = tokenize_dataset(val_dataset,tokenizer,max_length,mask_token,mask_prob)
    
    train_loader = DataLoader(train_dataset, batch_size=batch_size)
    val_loader   = DataLoader(train_dataset, batch_size=batch_size)
    return train_loader,val_loader

def tokenize_dataset(dataset,tokenizer,max_length,mask_token,mask_prob):
    ...
    masked_dataset = tokenized_dataset.map(mask_batch,mask_token,mask_prob)
...
def mask_batch(batch,mask_token,mask_prob):
    label = batch.copy()
